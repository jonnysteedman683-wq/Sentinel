...
# 1. Update activeTab initial value
code = code.replace(
...
aside_pattern = r"(<aside\b[^>]*>.*?<div className=\"p-6 flex-1 overflow-y-auto\">)(.*?)(</aside>)"
...
main_pattern = r"(<main\b[^>]*>.*?{/\* Chat Stream & Orb Container \*/}\n\s*<div className=\"flex-1 overflow-hidden relative flex flex-col\">)(.*?)(</main>)"
...
main_match = re.search(main_pattern, code, re.DOTALL)
if main_match:
    code = code[:main_match.start()] + new_main + code[main_match.end():]
else:

    ...
    <>
                {chatLayout === 'canvas' ? (
                  <div className="flex-grow w-full h-[550px] min-h-[400px] p-4 overflow-hidden relative z-10">
                    <Suspense fallback={<LazyFallback />}><CognitiveCanvas messages={messages.map((m: any) => ({ id: m.id, role: m.role, content: typeof m.content === 'string' ? m.content : JSON.stringify(m.content) || '', timestamp: m.timestamp ?? Date.now() }))} /></Suspense>
                  </div>
                ) : (
                              ...
                              <div className="flex items-start gap-4">
                                ...
                                <div className={`leading-relaxed ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
                                  <TypewriterText
                                    ...
                                    onComplete={() => {
                                      setMessages((prev: any) => prev.map((m: any) => m.id === msg.id ? { ...m, isTyping: false } : m))
                                    }}
                                  />
                                </div>
                              </div>
                          ...
                          <SelfHealingErrorCard
                            ...
                            onOfflineSimulate={() => {
                              ...
                              if (user) {
                                ...
                                setMessages((prev: any) => [...prev, { id: `mock-${Date.now()}`, ...mockAiData }]);
                              }
                            }}
                            onRetry={async () => {

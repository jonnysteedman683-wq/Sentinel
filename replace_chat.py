...
chat_pattern = re.compile(r'app\.post\("/api/chat", async \(req, res, next\) => \{.*?\n  \}\);\n\n  // Debate API route', re.DOTALL)
...
chat_code = '''app.post("/api/chat", async (req, res, next) => {
...
content = chat_pattern.sub(chat_code.replace('\\', '\\\\'), content)
